...
- ストローク開始時に最新カメラフレームを 1 枚の静止画として固定する。
- 固定した静止画から `FrameCapture` を使ってブラシ断面を生成する。
- 端末移動に合わせて、固定静止画上のブラシ断面サンプリング位置を動かす。

`ARSessionManager` は `@MainActor` / `@Observable` で、SwiftUI 側から最新状態を監視できるようにしています。

### `FrameCapture`

AR カメラフレーム、またはストローク開始時に固定した静止画からブラシ断面画像を生成します。

現在の断面生成は次の流れです。

1. ストローク開始時に `CVPixelBuffer` を縦持ち向きの `CGImage` として固定する。
2. 現在の端末移動から求めた画面上のストローク位置を、固定画像上のサンプリング位置へ変換する。
3. ブラシ角度に合わせて固定画像を回転する。
4. 現在のサンプリング位置を通る幅 1px の縦断面を切り出す。
5. `1 x 320` の `CGImage` として返す。
...
AR フレームごとに、開始 transform から現在 transform までの相対移動量を求め、開始時点の画面に並行な平面へ投影します。その投影点を、その時点のデバイス角度デルタで 2D 回転してから `ScreenStrokeSample` に保存します。ストロークサンプルは画面上の `x/y` 正規化座標だけを持ち、奥行き方向の `z` 変化量は保存も描画もしません。ブラシ角度は、初回ブラシ角度に開始 transform から現在 transform までのデバイス角度デルタを足して更新します。
...
この変換では `relativeTranslation.z` を意図的に捨てます。現在の実機確認で「順方向」として扱っている軸対応は `relativeY -> screenX`、`relativeX -> screenY` です。過去サンプルを表示時に再回転するのではなく、サンプル追加時点で 2D 平面座標へ定着させます。必要に応じてこの 1 行や回転方向を変えることで、上下左右の反転や回転補正を調整できます。
...
1. ストローク全体の輪郭を作り、描画をその輪郭内へクリップする。
2. 隣り合う 2 つの `ScreenStrokeSample` を取り出す。
3. 2 点間を 1pt 未満の細い slice に分割する。
4. 各 slice で位置・幅・ブラシ角度を補間する。
5. 前後サンプルの断面画像を補間しながら、細い line slice として描画する。
...
4. `ARSessionManager` が `latestPose` を更新する。ブラシ断面はこの段階では生成しない。
...
5. `ContentView` が `ARSessionManager.captureStrokeSourceImage()` を呼び、その時点のカメラフレームを静止画として固定する。
6. `ContentView` が `ScreenStrokeRecorder.begin(at:in:pose:brushAngleRadians:)` を呼ぶ。
7. AR プレビューは非表示になり、黒背景になる。
...
4. `ContentView` がストローク開始時に固定した静止画から、現在のストローク位置とブラシ角度に対応するカメラ断面を切り出す。
5. 必要な間隔を満たしていれば `ScreenStrokeSample` を追加する。
6. `ScreenStrokeRecorder.displayStrokes` が、描画時点で平面へ定着済みのアクティブストロークを返す。
...
- ストローク開始時に画像素材は固定しますが、断面のサンプリング位置は端末移動に合わせて更新します。
- ストローク中の断面画像は開始時点の静止画から作ります。ライブ動画から毎フレーム断面を作る設計ではありません。
...
- デバイス角度デルタを新規サンプルの平面座標へ焼き込むこと
- 既存サンプルが後続のデバイス回転で再投影されないこと
- ドラッグ方向の左 0 度定義
- ドラッグ開始位置からカメラ画像座標への変換
- aspect-fill クロップ補正
- 固定静止画からブラシ断面を切り出せること
- 端末移動に合わせてブラシ断面のサンプリング位置が動くこと
- ストローク終了時の描画順保持
...
- 画像素材としてのカメラフレームはストローク開始時に固定し、断面のサンプリング位置だけを端末移動に合わせて動かします。
