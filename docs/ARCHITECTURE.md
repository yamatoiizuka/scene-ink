...
同じ入力モデルは Web でも成立するため、`web/` に静的なブラウザ版プロトタイプも置いています。Web 版は `getUserMedia` と pointer events だけで動きます。
...
line の長さは画面上のブラシ幅をカメラ画像上の pixel 長へ変換して決めます。全画面分の line をブラシ幅へ縮小せず、ドラッグした局所範囲だけを素材にします。
...
`web/` はビルド不要の静的プロトタイプです。

```sh
python3 -m http.server 5174 --directory web
```

`http://127.0.0.1:5174/` を開くと、ブラウザのカメラ入力からドラッグ位置の局所 line をサンプリングして canvas に描画します。カメラ権限の都合で、配布時は HTTPS か localhost origin が必要です。
...
- 画面座標からカメラ画像座標への変換
- 画面上のブラシ幅をカメラ画像上の局所 line 長へ変換すること
- aspect-fill crop 補正
