...
- 録画中に現在のカメラフレームから `FrameCapture` を使ってブラシ断面を生成する。
- ストローク開始時に指定されたドラッグ開始位置を、ライブカメラ上の断面サンプリング位置として保持する。
...
2. ストローク開始時の画面 line 位置を、現在のカメラ画像上のサンプリング位置へ変換する。
3. 断面 line の周辺だけを小さく crop する。
4. ブラシ角度に合わせて crop 済みパッチを回転する。
...
4. `ARSessionManager` が `latestPose` を更新する。ブラシ断面は録画中に必要なタイミングだけ生成する。
...
4. `ContentView` が現在のカメラフレームから、ストローク開始時の画面 line と現在ブラシ角度に対応するカメラ断面を切り出す。
5. 必要な間隔を満たしていれば `ScreenStrokeSample` を追加する。間隔を満たさない AR フレームでは、ブラシ断面の切り出し自体を行わない。
...
- ストローク中の断面画像はライブカメラフレームから作ります。ただし全画面を回転せず、断面 line 周辺の小さなパッチだけを処理します。
...
- ブラシ断面処理が line 周辺の小さな crop に制限されること
- サンプル追加しない AR フレームではブラシ断面を要求しないこと
- ストローク終了時の描画順保持
...
- 画像素材はライブカメラフレームを使います。処理負荷を抑えるため、断面 line 周辺だけを crop してから回転・断面化します。
