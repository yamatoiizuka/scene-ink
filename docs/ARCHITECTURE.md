...
- 左手: ロータリー UI でブラシ太さとブラシ角度を調整する。
- 右手: カメラプレビュー上をタップして、サンプリング位置とストローク開始位置を決める。
- 右手でもう一度タップ: 現在のストロークを終了する。
...
- `StrokeTouchSurface` で右手タップを受け、ストローク開始/終了を切り替える。
- `RotaryBrushControl` でブラシ太さと角度を更新する。
...
- ストローク開始時に指定された画面タップ位置を、ブラシ断面のサンプリング位置として保持する。
...
3. 画面タップ位置をカメラ画像上のサンプリング位置へ変換する。
4. ブラシ角度に合わせて画像を回転する。
...
AR フレームごとに、開始 transform から現在 transform までの相対移動量を求め、画面座標へ投影します。

```swift
screenTranslation = SIMD2(relativeTranslation.y, relativeTranslation.x)
```

この変換は、現在の実機確認で「順方向」として扱っている軸対応です。必要に応じてこの 1 行を変えることで、上下左右の反転や回転補正を調整できます。
...
- 中心からの距離をブラシ太さに変換する。
- 中心から見て左方向を 0 度とする。
- 下方向は 90 度、右方向は 180 度、上方向は 270 度に対応する。
- 幅は `1...96px` の整数で、1px 単位に丸める。

角度はブラシ描画の断面方向と、カメラ画像からサンプリングする断面角度の両方に使われます。

### `StrokeTouchSurface`

画面全体に重ねる透明な UIKit view です。ロータリー UI 以外の領域で右手タップを受け取る役割です。

現在はドラッグではなくタップだけを扱います。`touchesBegan` と `touchesEnded` の距離が 22pt 未満ならタップとみなし、`ContentView` に座標と view size を渡します。
...
`ScreenStrokeSample` の配列です。1 回の開始タップから終了タップまでが 1 ストロークです。
...
1. ユーザーが背景カメラ画像上をタップする。
2. `StrokeTouchSurface` がタップ座標と view size を返す。
3. `ContentView` が `ARSessionManager.setBrushSamplePoint(point, in: size)` を呼ぶ。
4. `ContentView` が `ScreenStrokeRecorder.begin(at:in:pose:)` を呼ぶ。
5. AR プレビューは非表示になり、黒背景になる。

この時点で、タップ位置は「画面上のストローク開始位置」と「カメラ画像のサンプリング位置」の両方として使われます。

### ストローク中

1. AR フレーム更新ごとに `latestPose` が変わる。
2. `ContentView.onChange(of: latestPose.timestamp)` が発火する。
3. `ScreenStrokeRecorder.record(...)` が現在 pose から画面上の点を計算する。
4. 必要な間隔を満たしていれば `ScreenStrokeSample` を追加する。
5. `StrokeCanvasView` がアクティブストロークを再描画する。
...
ロータリー UI では左方向を 0 度とします。内部ではラジアンで保持します。
...
タップ座標は `normalizedPreviewPoint` として `0...1` に正規化されます。`FrameCapture.sourcePoint(...)` は、プレビュー view の aspect-fill クロップを考慮して、カメラ画像上の点へ変換します。
...
- タップ位置の背景画像がストローク断面に反映されるか
- 端末移動方向とストローク方向が意図どおりか
- ブラシ角度変更時にサンプリング断面と描画断面が同時に変わるか
- 複数ストロークの重なり順が自然か
